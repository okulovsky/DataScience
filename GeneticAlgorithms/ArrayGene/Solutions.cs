...
namespace DataScience.GeneticAlgorithms
{
    partial class Solutions
    {
        ...
        public partial class Mutators
        {
            ...
            public static void CutPaste<G, T>(GeneticAlgorithm<G> alg)
                where G : ArrayChromosome<T>
                where T : IEquatable<T>
            {
                alg.PerformMutation = delegate (G source)
                {
                    ...
                    var pos1 = alg.Rnd.RandomInt(g.Code.Length);
                    var pos2 = alg.Rnd.RandomInt(g.Code.Length);
                    var value = g.Code[pos1];
                    for (int i = pos1; i < g.Code.Length - 1; i++)
                        g.Code[i] = g.Code[i + 1];
                    for (int i = g.Code.Length-1; i > pos2; i--)
                        g.Code[i] = g.Code[i - 1];
                    g.Code[pos2] = value;
                    return g;
                };
        ...
        public partial class Appearences
        {
